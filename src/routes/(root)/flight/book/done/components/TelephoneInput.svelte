<script lang="ts">
	...
	export let handlePhoneChange = (event: Event) => {
		...
		dispatch('phoneChange', { fullPhoneNumber: `${selectedCountry.dialCode}${phoneNumber}` });
	};
...
<div class="grid">
	<div class="flex relative countries border {error ? 'border-red-500' : 'border-neutral-300'} ">
		...
		class="w-fit max-w-[100px] relative border border-neutral-300 bg-neutral-100 p-2 text-sm focus:outline-none"
	>
		{#each countries as country}
			<option
				value={country.dialCode}
				selected={country.dialCode === selectedCode}
				class="relative w-24 flex items-center"
			>
	...
	{#if error}
		<span class="text-xs text-red-500 font-bold">Invalid phone</span>
	{/if}
</div>
